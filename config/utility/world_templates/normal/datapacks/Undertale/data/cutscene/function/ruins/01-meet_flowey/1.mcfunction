...
scoreboard players set #01-meet-flowey cutscene 1
execute if data storage data {remembers:0} run data merge storage data {has_met_flowey:1}
...
execute scheduled 6t run execute if score #01-meet-flowey cutscene matches 1 run model entity @n[type=minecraft:item_display,tag=floweycs1] container.0 animation play wave
...
execute scheduled 5 run textBox display Twishorts 14069001 {"text": "* TWISHORTS??\n  WHAT ARE YOU DOING HERE?????????????\n  ??????????????????????????????????????????\n  ??????????????????????????????????????????????","font":"customfonts:determination","color": "white"}
textBox display @s 14069001 [{"text": "","font":"customfonts:determination","color": "white"}]

textBox style 14069000 image texture minecraft:block/character/flowey/box/smiletalk
execute scheduled 5 run execute scheduled 45 run execute if score #01-meet-flowey cutscene matches 1 run textBox style 14069000 image texture minecraft:block/character/flowey/box/smile

execute scheduled 5 run execute scheduled 45 run execute if score #01-meet-flowey cutscene matches 1 run item replace entity @n[type=minecraft:item_display,tag=floweycs1] container.0 with minecraft:netherite_block[minecraft:custom_model_data=90050]

execute scheduled 5 run execute scheduled 45 run execute if score #01-meet-flowey cutscene matches 1 run scoreboard players set @s canProgress 1

execute scheduled 7 run sound modify @s atTarget 6 volume 0
execute scheduled 7 if data storage data {voices:1} scheduled 1t run sound play @s atTarget 6 minecraft:dialogue.cutscene1.1 voice 1 1 false 0 false