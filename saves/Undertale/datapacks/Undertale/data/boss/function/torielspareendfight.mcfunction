...
execute scheduled 13t as @a at @s run scoreboard players set #canmove cutscene 0
...
execute scheduled 14t run function cutscene:ruins/18-toriel_fight/5/1
execute scheduled 9t run tp @s -832.0 47.0 53.0 0 0

item replace entity @n[type=minecraft:item_display,tag=TorielBattle] container.0 with minecraft:netherite_block[minecraft:custom_model_data=90084,utility:utility_animation={current_animation: "idle", start_time: 44373318L}]
scoreboard players set #torielfight data 5
scoreboard players add #torielspared data 1