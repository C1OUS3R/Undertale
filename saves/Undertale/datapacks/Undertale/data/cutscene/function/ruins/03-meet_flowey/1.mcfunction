...
scoreboard players set @s cutscene 3
scoreboard players set @s canDance 0
scoreboard players set @s canProgress 0

scoreboard players set #03-meet-flowey cutscene 1
...
execute scheduled 5 run textBox display @s 14069001 [{"text": "* Don't you have anything\n  better to do?","font":"customfonts:determination"}]
execute unless gameValue HasShayyLoggedOn 1 scheduled 5 run textBox display _shayy 14069001 [{"text": "* Shayy it's late,\n  go to bed.","font":"customfonts:determination"}]

textBox style 14069000 image texture minecraft:block/character/flowey/box/smiletalk
execute scheduled 5 run execute scheduled 30 run execute if score #03-meet-flowey cutscene matches 1 run textBox style 14069000 image texture minecraft:block/character/flowey/box/smile

execute scheduled 5 run execute scheduled 30 run execute if score #03-meet-flowey cutscene matches 1 run item replace entity @n[type=minecraft:item_display,tag=floweycs1] container.0 with minecraft:netherite_block[minecraft:custom_model_data=90050]

execute scheduled 5 run execute scheduled 30 run execute if score #03-meet-flowey cutscene matches 1 run scoreboard players set @s canProgress 1

execute scheduled 7 run sound modify @s atTarget 6 volume 0
execute scheduled 7 if data storage data {voices:1} scheduled 1t run sound play @s atTarget 6 minecraft:dialogue.cutscene3.1 voice 1 1 false 0 false

execute scheduled 10 run execute if entity @s[name=_shayy] run gameValue HasShayyLoggedOn set 1