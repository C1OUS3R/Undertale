...
scoreboard players set #kill_toriel_data data 3
...
execute scheduled 5 run textBox display @s 14069001 [{"text": "* Wow, you really\n  can't get enough.","font":"customfonts:determination"}]

textBox style 14069000 image texture minecraft:block/character/flowey/box/smiletalk
execute scheduled 5 run execute scheduled 40 run execute if score #22-meet-flowey cutscene matches 1 run textBox style 14069000 image texture minecraft:block/character/flowey/box/smile

execute scheduled 5 run execute scheduled 40 run execute if score #22-meet-flowey cutscene matches 1 run item replace entity @n[type=minecraft:item_display,tag=floweycs22] container.0 with minecraft:netherite_block[minecraft:custom_model_data=90050]

execute scheduled 5 run execute scheduled 40 run execute if score #22-meet-flowey cutscene matches 1 run scoreboard players set @s canProgress 1

execute scheduled 7 run sound modify @s atTarget 6 volume 0
execute scheduled 7 if data storage data {voices:1} scheduled 1t run sound play @s atTarget 6 minecraft:dialogue.cutscene22.17 voice 1 1 false 0 false