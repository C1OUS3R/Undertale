##
 # set1.mcfunction
 # 
 #
 # Created by .
##

sound play @s atTarget -5 minecraft:sfx.levelup block 1 1 false 0 false

scoreboard players set @a lv 9
scoreboard players set @a maxHealth 52

scoreboard players set @a atk 26
scoreboard players set @a def 11
scoreboard players set @a neededExp 1200
scoreboard players set @a levelup 1
execute scheduled 1s run scoreboard players reset @a levelup

scoreboard players set @a playerBaseAttack 16
scoreboard players set @a playerBaseDefense 2
